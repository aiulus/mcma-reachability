function [sys, R0, U, p_true] = extended_loadDynamics(dynamics, dt, dim, type, p)
...
switch dynamics
    ...
    case "tank60"
        ...
        case 'chain_of_integrators'
            ...
            c_R0 = zeros(size(A, 1),1);
            G_R0 = [];
            c_U = 0.1+zeros(size(B, 2),1);
            G_U = 0.2*diag(ones(size(B, 2),1));
            c_V =  -0.05+zeros(size(C, 1),1);
            G_V = 0.1*[diag(ones(size(C, 1),1)) ones(size(C, 1),1)];
            R0 = zonotope([c_R0,G_R0]);
            V = zonotope([c_V,G_V]);
            U = cartProd(zonotope([c_U,G_U]), V);
