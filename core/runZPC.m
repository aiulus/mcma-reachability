function [uPred, uPred_model, y_t, y_t_model, execTimeZPC, execTimeRMPC] = ...
    ...
    for k = 1:maxsteps
        ...
        [uPred(k), YPred(:,k+1), u, y, execTimeZPC(k), Rplotall{k}] = ...
            solveZPC(y_t(:,k), sys_d, N, r_u, r_y, U, Qy, Qu, W, V, AV, intc, k, maxsteps);
        ...
        for i = 1:N
            ...
            R{i+1} = zonotope([ABcard.center,[ABcard.generators,W.generators,V.generators,AV.generators]]); %A B * card_zono + W_sdp;

            %convert R to interval           
            c = R{i+1}.Z(:,1);  %extract center             
            delta = sum(abs(R{i+1}.Z),2) - abs(c);
            leftLimit{i} = c - delta;
