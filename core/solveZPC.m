function [u_opt, y_next, u, y, execTime, R_plot] = ...
    solveZPC(current_y, sys_d, N, r_u, r_y, U, Qy, Qu, W, V, AV, intc, k, maxsteps)
    ...
    alpha_u = sdpvar(1,N);
    s_inf    = sdpvar(5*ones(1,N+1), ones(1,N+1));
    s_sup    = sdpvar(5*ones(1,N+1), ones(1,N+1));

    % ---- Initialize reachable sets ----
    R = cell(1, N+1);
    R{1} = zonotope(current_y);         % Original: R{1} = zonotope([y_t(:,k)]);
